...
export function ActiveFilters({ filters, onRemove }: ActiveFiltersProps) {
  ...
  if (filters.dateRange.from || filters.dateRange.to) {
    ...
    if (filters.dateRange.preset && filters.dateRange.preset !== 'custom') {
      label =
        DATE_PRESET_LABELS[filters.dateRange.preset] ||
        filters.dateRange.preset;
    } else if (filters.dateRange.from && filters.dateRange.to) {
      label = `${format(filters.dateRange.from, 'MMM d')} - ${format(
  ...
  if (filters.specificChat !== null) {
    badges.push({
      key: 'specificChat',
      label: `Chat #${filters.specificChat}`,
    });
  }
      ...
      <div className="flex flex-wrap gap-1.5 -ml-1">
        {badges.map((badge) => (
          <Badge
            key={badge.key}
            variant="secondary"
            className="cursor-pointer py-3 gap-1 pr-1"
