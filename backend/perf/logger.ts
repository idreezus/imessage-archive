import { isPerfEnabled, PerfCategory } from "./config";
...
/**
 * Log a startup sequence table showing all phases and total time.
 */
export function logStartupTable(
  phases: { name: string; duration: number }[]
): void {
  if (!isPerfEnabled()) return;

  const ts = getTimestamp();
  console.log(`[PERF] [${ts}] [STARTUP] ============ App Startup ============`);
  phases.forEach(({ name, duration }) => {
    console.log(`[PERF] [${ts}] [STARTUP] ${name}: ${duration}ms`);
  });
  console.log(`[PERF] [${ts}] [STARTUP] ====================================`);
  const total = phases.reduce((sum, p) => sum + p.duration, 0);
  console.log(`[PERF] [${ts}] [STARTUP] total: ${total}ms`);
}
